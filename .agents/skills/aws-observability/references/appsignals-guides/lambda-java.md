...
> Lambda — and it is genuinely scoped rather than broad: its `logs:` actions are limited to
...
> `references/instrumentation/instrumentation.md` states it is out of scope there. That path grants
...
> the two-action inline form is the optional least-privilege alternative, not the default. Note the
> two policies are not interchangeable in either direction: this one does **not** grant
