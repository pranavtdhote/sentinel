...
> The `0.0.0.0` receivers below listen on every interface with no TLS and no authentication. Bind
> them to `127.0.0.1` when the senders are on the same host; otherwise it is recommended to restrict
> the OTLP `4317` / `4318` ports via security groups or host firewall and to avoid co-locating
> untrusted workloads. This guide does not apply those controls, so assess and configure them for
> your environment.
