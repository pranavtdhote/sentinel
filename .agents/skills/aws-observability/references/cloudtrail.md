...
> ```
>
> **Check which selector style the trail uses first.** `get-event-selectors` returns *either*
> `AdvancedEventSelectors` *or* the basic `EventSelectors`, and `put-event-selectors` accepts only one
