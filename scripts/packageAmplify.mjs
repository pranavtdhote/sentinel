...
console.log('=== Step 6: Generating deploy-manifest.json ===');
const manifest = {
  version: 1,
  framework: {
    name: 'next',
...
console.log('=== Step 7: Creating deployment zip archive ===');
...
try {
  console.log('Compressing with tar from inside .amplify-hosting...');
  execSync('tar -a -c -f sentinel-deployment.zip -C .amplify-hosting .', { stdio: 'inherit' });
} catch (e) {
  console.log('tar failed, falling back to powershell Compress-Archive...');
  execSync('powershell -Command "Compress-Archive -Path .amplify-hosting/* -DestinationPath sentinel-deployment.zip -Force"', { stdio: 'inherit' });
}
